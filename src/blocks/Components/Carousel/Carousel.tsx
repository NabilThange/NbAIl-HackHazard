...
export default function Carousel({
  ...
  return (
    <div
      ref={containerRef}
      className={`
        ${responsive ? 'w-full max-w-md mx-auto' : 'relative overflow-hidden'} 
        ${round ? 'rounded-full border border-white' : 'rounded-[24px] border border-[#222]'}
        ${responsive ? 'md:hidden' : ''}
      `}
      style={{
        width: responsive ? '95%' : `${baseWidth}px`,
        maxWidth: responsive ? '400px' : undefined,
        height: responsive ? 'auto' : (round ? `${baseWidth}px` : undefined),
        aspectRatio: responsive ? '1 / 1.2' : undefined,
      }}
    >
      <motion.div
        className="flex"
        ...
        {...dragProps}
        style={{
          width: responsive ? '100%' : itemWidth,
          gap: `${GAP}px`,
          perspective: 1000,
